...
export const AdminAuditLogsPage: React.FC = () => {
  ...
  const getActionBadge = (action: string) => {
    ...
    switch (act) {
      ...
      default:
        return <span className="px-2 py-0.5 rounded-md text-[10px] font-bold bg-slate-800 text-slate-300 border border-slate-700">{act || "ACTION"}</span>
    }
  }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="p-4 rounded-2xl bg-slate-900 border border-slate-800 shadow-lg">
          ...
          <p className="text-2xl font-black text-white mt-2">{auditLogs.length}</p>
          <p className="text-[11px] text-slate-400 mt-1">Cross-tenant transactions</p>
        </div>
        ...
        <div className="p-4 rounded-2xl bg-slate-900 border border-slate-800 shadow-lg">
          ...
          <p className="text-[11px] text-slate-400 mt-1">Inspected & validated</p>
        </div>
        ...
        <div className="p-4 rounded-2xl bg-slate-900 border border-slate-800 shadow-lg">
          ...
          <p className="text-[11px] text-slate-400 mt-1">Auth & credential events</p>
        </div>
        ...
        <div className="p-4 rounded-2xl bg-slate-900 border border-slate-800 shadow-lg">
          ...
          <p className="text-2xl font-black text-violet-400 mt-2">100%</p>
          <p className="text-[11px] text-slate-400 mt-1">Cryptographically intact</p>
        </div>
      </div>
      ...
      <div className="p-4 rounded-2xl bg-slate-900 border border-slate-800 flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3 shadow-lg">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <select
            ...
            className="px-3 py-2 text-xs rounded-xl bg-slate-950 border border-slate-800 text-slate-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 max-w-[200px]"
          >
                ...
                filteredLogs.map((log) => (
                  <tr key={log.id} className="hover:bg-slate-800/40 transition-colors group">
                    ...
                    <td className="py-3.5 px-4">
                      <div className="font-bold text-slate-100">{log.actor_name || "System"}</div>
                      <div className="text-[11px] text-slate-400 font-mono">{log.actor_email || "system@omniplatform.internal"}</div>
                    </td>

                    {/* Target Scope */}
                    <td className="py-3.5 px-4">
                      <div className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md text-[11px] font-medium bg-slate-950 border border-slate-800 text-slate-300">
                    ...
                    <td className="py-3.5 px-4">
                      ...
                      <div className="text-[10px] text-slate-400 font-mono mt-0.5">
                        Type: <span className="text-indigo-400 font-bold">{log.resource_type || "N/A"}</span>
                    ...
                    <td className="py-3.5 px-4 text-right">
                      <button
                        ...
                        className="p-1.5 rounded-lg bg-slate-800 hover:bg-indigo-950/40 text-slate-300 hover:text-indigo-300 border border-slate-700 hover:border-indigo-800 transition-colors"
          ...
          <div className="space-y-4 pt-2">
            ...
            <div className="grid grid-cols-2 gap-3 p-3.5 rounded-xl bg-slate-950 border border-slate-800 text-xs">
              ...
              <div>
                ...
                <span className="font-bold text-indigo-400">{selectedLog.resource_type} ({selectedLog.resource_id || "N/A"})</span>
              </div>
              <div>
                <span className="text-slate-500">Actor:</span>{" "}
                <span className="font-bold text-slate-200">{selectedLog.actor_email || "System"}</span>
              </div>
              <div>
              ...
              <div>
                ...
                <span className="font-semibold text-slate-300">{selectedLog.tenant_name || "Platform-Wide"}</span>
              </div>
              <div>
              ...
              <div className="col-span-2">
                ...
                <span className="font-mono text-[10px] text-slate-400 truncate block">{selectedLog.user_agent || "Antigravity/REST-Client"}</span>
              </div>
            </div>

            {/* Description note */}
            {selectedLog.description && (
              <div className="p-3 rounded-xl bg-indigo-950/40 border border-indigo-900/60 text-xs text-indigo-300">
                <span className="font-bold text-indigo-200">Event Description:</span> {selectedLog.description}
              </div>
            )}
            ...
            <div>
              <div className="flex items-center justify-between pb-1.5">
                <span className="text-xs font-bold text-slate-300 flex items-center gap-1.5">
                  <Code className="w-3.5 h-3.5 text-indigo-400" /> Changes Payload & State Differences
                </span>
                <span className="text-[10px] font-mono text-slate-500">JSON Format</span>
              </div>
              <pre className="p-4 rounded-xl bg-slate-950 border border-slate-800 text-indigo-300 font-mono text-xs overflow-x-auto max-h-72 leading-relaxed">
