...
function mapBackendStaff(s: any): Staff {
  ...
  return {
    ...
    firstName: fn,
    lastName: ln,
    email: s.email || s.user?.email || (fn ? `${fn.toLowerCase()}.${ln.toLowerCase()}@omni-edu.org` : "—"),
    avatar: s.avatar_url || s.user?.avatar_url || `https://ui-avatars.com/api/?name=${encodeURIComponent(fn)}+${encodeURIComponent(ln)}&background=3F72AF&color=fff`,
    roleTitle: s.designation || "Faculty",
    department: s.department_name || s.department?.name || "—",
    designation: s.designation || "—",
    joiningDate: s.joined_date || s.created_at?.split("T")[0] || "",
